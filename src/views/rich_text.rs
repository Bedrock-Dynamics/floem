...
impl RichText {
    ...
    fn selected_text(&self, start: &Cursor, end: &Cursor) -> String {
        ...
        for i in start.line..=end.line {
            ...
            let to = if i == end.line { end.index } else { lt.len() };
        ...
        match command {
            TextCommand::Copy => {
                if let Some((start_c, end_c)) = &self.selection_range {
                    let txt = self.selected_text(start_c, end_c);
                    if !txt.is_empty() {
...
impl View for RichText {
    ...
    fn paint(&mut self, cx: &mut crate::context::PaintCx) {
        ...
        // same frame).
        let show_selection = cx.app_state.is_focused(&self.id())
            || matches!(self.selection_state, SelectionState::Selecting(_, _));
        if show_selection {
